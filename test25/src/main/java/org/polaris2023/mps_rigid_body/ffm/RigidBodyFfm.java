...
public final class RigidBodyFfm {
    ...
    private final MethodHandle rigidBodyBuilderSetAdditionalMass;
    ...
    private final MethodHandle worldCreateSharedArena;
    private final MethodHandle worldDestroySharedArena;
    ...
    public RigidBodyFfm(Path library, Arena arena) {
        ...
        rigidBodyBuilderSetAdditionalMass = downcall("rigid_body_builder_set_additional_mass", FunctionDescriptor.ofVoid(ValueLayout.ADDRESS, ValueLayout.JAVA_DOUBLE));
        ...
        worldCreateSharedArena = downcall("world_create_shared_arena",
                FunctionDescriptor.of(BOOL, ValueLayout.ADDRESS, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.ADDRESS));
        worldDestroySharedArena = downcall("world_destroy_shared_arena",
                FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
    ...
    public boolean worldCreateSharedArena(MemorySegment world, int maxBodies, int maxColliders, int maxEvents, int maxCommands, MemorySegment outAddress, MemorySegment outSize) {
        try {
            SegmentAllocator allocator = arena;
            return boolValue((MemorySegment) worldCreateSharedArena.invokeExact(allocator, world, maxBodies, maxColliders, maxEvents, maxCommands, outAddress, outSize));
        } catch (Throwable throwable) {
            throw callFailed("world_create_shared_arena", throwable);
        }
    }

    public void worldDestroySharedArena(MemorySegment world) {
        try {
            worldDestroySharedArena.invokeExact(world);
        } catch (Throwable throwable) {
            throw callFailed("world_destroy_shared_arena", throwable);
        }
    }
    ...
    public void rigidBodyBuilderSetAdditionalMass(MemorySegment builder, double mass) {
        try {
            rigidBodyBuilderSetAdditionalMass.invokeExact(builder, mass);
        } catch (Throwable throwable) {
            throw callFailed("rigid_body_builder_set_additional_mass", throwable);
        }
    }
