...
pub fn hookes_law_uniaxial(stress: f64, youngs_modulus: f64) -> Option<f64> {
    if !stress.is_finite() || !youngs_modulus.is_finite() || youngs_modulus <= 0.0 {
        return None;
    }
...
pub fn stress_from_strain(youngs_modulus: f64, strain: f64) -> Option<f64> {
    if !youngs_modulus.is_finite() || youngs_modulus <= 0.0 || !strain.is_finite() {
        return None;
    }
...
pub fn shear_modulus(youngs_modulus: f64, poisson_ratio: f64) -> Option<f64> {
    if !youngs_modulus.is_finite()
        ...
        || !(-1.0..=0.5).contains(&poisson_ratio)
    {
        return None;
    }
...
pub fn lame_mu(shear_modulus: f64) -> f64 {
    shear_modulus
}
...
pub fn plane_stress_stiffness(youngs_modulus: f64, poisson_ratio: f64) -> Option<(f64, f64, f64)> {
    if !youngs_modulus.is_finite()
        || youngs_modulus <= 0.0
        || !poisson_ratio.is_finite()
        || !(-1.0..1.0).contains(&poisson_ratio)
    {
        return None;
    }
...
pub fn von_mises_yield_check(von_mises_stress: f64, yield_stress: f64) -> Option<f64> {
    if !von_mises_stress.is_finite()
        || von_mises_stress < 0.0
        || !yield_stress.is_finite()
        || yield_stress <= 0.0
    {
        return None;
    }
...
pub fn tresca_shear_stress(sigma_1: f64, sigma_3: f64) -> Option<f64> {
    if !sigma_1.is_finite() || !sigma_3.is_finite() {
        return None;
    }
...
pub fn principal_stresses(
    ...
    let i3 =
        sx * sy * sz + 2.0 * txy * tyz * tzx - sx * tyz * tyz - sy * tzx * tzx - sz * txy * txy;
    // Depressed cubic: y³ - p·y - q = 0 where y = σ - I₁/3
    let p = i1 * i1 / 3.0 - i2;
    let q = 2.0 * i1 * i1 * i1 / 27.0 - i1 * i2 / 3.0 + i3;
    if p < 0.0 {
        return None;
    } // should not happen for real stress tensors
...
pub fn ki_center_crack(stress: f64, crack_half_length: f64) -> Option<f64> {
    if !stress.is_finite() || !crack_half_length.is_finite() || crack_half_length <= 0.0 {
        return None;
    }
...
pub fn ki_edge_crack(stress: f64, crack_length: f64) -> Option<f64> {
    if !stress.is_finite() || !crack_length.is_finite() || crack_length <= 0.0 {
        return None;
    }
...
pub fn fracture_check(stress_intensity: f64, fracture_toughness: f64) -> Option<f64> {
    if !stress_intensity.is_finite()
        || stress_intensity < 0.0
        || !fracture_toughness.is_finite()
...
pub fn basquin_stress_amplitude(
    cycles_to_failure: f64,
    fatigue_strength_coefficient: f64,
    fatigue_exponent: f64,
) -> Option<f64> {
    if !cycles_to_failure.is_finite()
        || cycles_to_failure < 1.0
        || !fatigue_strength_coefficient.is_finite()
        || fatigue_strength_coefficient <= 0.0
        || !fatigue_exponent.is_finite()
    {
        return None;
    }
...
pub fn coffin_manson_strain_amplitude(
    cycles_to_failure: f64,
    ...
    if !cycles_to_failure.is_finite()
        || cycles_to_failure < 1.0
        || !ductility_coefficient.is_finite()
        || ductility_coefficient <= 0.0
        || !ductility_exponent.is_finite()
    {
        return None;
    }
...
pub fn goodman_correction(
    ...
    if !stress_amplitude.is_finite()
        || stress_amplitude < 0.0
        || !mean_stress.is_finite()
        || !ultimate_tensile.is_finite()
        || ultimate_tensile <= 0.0
    {
        return None;
    }
...
pub fn beam_bending_stress(
    ...
    if !bending_moment.is_finite()
        || !distance_from_neutral_axis.is_finite()
        || distance_from_neutral_axis < 0.0
...
pub fn beam_deflection_center_point_load(
    load: f64,
    span: f64,
    youngs_modulus: f64,
...
pub fn euler_buckling_load(
    ...
    if !youngs_modulus.is_finite()
        ...
        || !column_length.is_finite()
        || column_length <= 0.0
    {
        return None;
    }
    Some(
        PI * PI * youngs_modulus * moment_of_inertia
            / (effective_length_factor * column_length).powi(2),
    )
}
...
pub fn slenderness_ratio(
    ...
    if !effective_length_factor.is_finite()
        || effective_length_factor <= 0.0
        || !column_length.is_finite()
        || column_length <= 0.0
