//! Trajectory dynamics — ballistic, drag/lift, and gliding trajectory formulas.
//!
...
/// Compute ballistic trajectory forces.
...
/// Integrate one step of ballistic trajectory.
pub fn integrate_step(
    state: &TrajectoryState,
    env: &TrajectoryEnvironment,
    dt: f64,
) -> Option<(TrajectoryState, TrajectoryForceReport)> {
    ...
    let velocity = crate::math::Vector3f64::new(
    ...
    let position = crate::math::Vector3f64::new(
...
/// Compute glide trajectory derivatives.
pub fn compute_glide_report(
...
/// Integrate one step of gliding trajectory using RK4.
pub fn integrate_glide_step(
    state: &TrajectoryGlideState,
    env: &TrajectoryGlideEnvironment,
    dt: f64,
) -> Option<(TrajectoryGlideState, TrajectoryGlideReport)> {
    if !dt.is_finite() || dt <= 0.0 || dt > MAX_STEP_SECONDS {
        return None;
    }

    let k1 = compute_glide_report(state, env)?;
    let k2 = compute_glide_report(&add_glide_scaled(state, &k1, dt * 0.5), env)?;
    let k3 = compute_glide_report(&add_glide_scaled(state, &k2, dt * 0.5), env)?;
    let k4 = compute_glide_report(&add_glide_scaled(state, &k3, dt), env)?;
