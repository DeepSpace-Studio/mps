...
pub const SCIENTIST: ScientistRecord = ScientistRecord {
    id: "leonhard_euler",
    name: "Leonhard Euler",
    birth_year: Some(1707),
    death_year: Some(1783),
    field_id: "mechanics",
    nationality: "Swiss",
    contribution: "Euler angles; rigid-body rotation; fluid & beam equations",
    key_constants: "",
};
...
pub mod formulas {
    ...
    pub fn sph_estimate_forces(
        ...
        for neighbor in particles {
            ...
            let diff = mul_add(-1.0_f64, distance, smoothing_radius);
            let gradient =
                -offset / distance * (45.0 / (PI * smoothing_radius.powi(6)) * diff * diff);
            pressure_force.add(vec3_from_rapier(
                -neighbor.mass
                    * ((pressure + neighbor_pressure) / (2.0 * neighbor_density))
                    * gradient,
            ));
