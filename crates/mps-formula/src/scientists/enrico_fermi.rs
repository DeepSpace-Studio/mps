...
pub const SCIENTIST: ScientistRecord = ScientistRecord {
    ...
    death_year: Some(1954),
    field_id: "quantum_mechanics",
    nationality: "Italian/American",
    contribution: "Fermi-Dirac; nuclear reactor; beta decay",
    key_constants: "fermion",
};
...
pub mod formulas {
    ...
    pub fn bateman_abundance(
        ...
        if !parent_initial.is_finite()
            || parent_initial < 0.0
            || !time.is_finite()
            || time < 0.0
            || n == 0
            || n > decay_constants.len()
        {
            return None;
        }
        ...
        let n = n - 1; // 0-indexed
        if n == 0 {
            return Some(parent_initial * (-lambdas[0] * time).exp());
        }
    ...
    pub fn neutron_flux_sphere(
        ...
        if !radius.is_finite()
            || radius <= 0.0
            || !diffusion_coefficient.is_finite()
            || diffusion_coefficient <= 0.0
            || !source_strength.is_finite()
            || source_strength < 0.0
            || !r.is_finite()
            || r < 0.0
            || r > radius
    ...
    pub fn macroscopic_cross_section(
        ...
        if !number_density.is_finite()
            || number_density < 0.0
            || !microscopic_cross_section.is_finite()
            || microscopic_cross_section < 0.0
        {
            return None;
        }
