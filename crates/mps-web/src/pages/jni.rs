        ...
        <div>
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:30px;padding-bottom:20px;border-bottom:1px solid #333;">
                <div>
                    ...
                    <p style="font-size:14px;color:#999;line-height:1.7;margin:0;">{ "mps-jni 编出 cdylib " }<code>"mps_rigid_body"</code>{ "，" }{ (JNI_METHOD_COUNT) }{ " 个 " }<code>"jni! / jni_e_c!"</code>{ " 宏导出方法分两块：通用物理走 mps-core 的 " }<code>"RigidBodyNative"</code>{ "（" }<code>"org.polaris2023.mps_rigid_body.RigidBodyNative"</code>{ "），太空刚体走 cosmos* 系列。每方法都由 catch_unwind 兜底——panic 永不进 JVM。" }</p>
                </div>
