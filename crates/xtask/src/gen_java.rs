            ...
            syn::Meta::List(ml) => {
                ...
                let _ = ml.parse_nested_meta(|meta| {
                    if meta.path.is_ident("package")
                        && let Ok(v) = meta.value()?.parse::<syn::LitStr>()
                    {
                        found = Some(v.value());
                    }
                    Ok(())
...
fn gen_enum_java(e: &EnumInfo) -> String {
    ...
    sb.push('\n');
    sb.push_str("    public static int fromRaw(int raw) {\n        return raw;\n    }\n\n");
    sb.push_str("    public static int toRaw(int value) {\n        return value;\n    }\n");
    sb.push_str("}\n");
    sb
}
